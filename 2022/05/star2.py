...
def crate_mover_9001(stack, quantity, source_queue, destination_queue):
    debug(f"stack before: {stack}")
    sq = deque(stack[source_queue-1])
    dq = deque(stack[destination_queue-1])
    tmp_queue = deque([])
    for item in range(quantity):
        tmp_queue.appendleft(sq.popleft())
    debug(f"tmp_queue: {tmp_queue}")
